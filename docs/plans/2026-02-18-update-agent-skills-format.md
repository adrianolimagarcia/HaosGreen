...
1. **`skills/`** — HaosGreen's own bot skills (loaded into the Telegram bot's LLM system prompt by `src/skills/loader.rs`). Currently uses flat `.md` files → convert to `skill-name/SKILL.md` folder format.
...
## Part A: HaosGreen Bot Skills (`skills/`)

### How HaosGreen Skills Work
...
HaosGreen skills also support an optional `tags` field in frontmatter (HaosGreen extension, not in Claude spec).
...
These are used by Claude Code when developing HaosGreen, not by the bot itself.
