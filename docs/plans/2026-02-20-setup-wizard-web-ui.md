...
echo "Starting HaosGreen setup wizard at http://localhost:$PORT"
...
```python
#!/usr/bin/env python3
"""Minimal HTTP server for HaosGreen setup wizard."""
import http.server
import json
import os
import sys
...
const state = {
  ...
  location: '',
  sandbox_dir: '/tmp/haos-green-sandbox',
  db_path: 'haos-green.db',
...
<div class="step" id="step-4">
  ...
  <div class="field">
    ...
    <input type="text" id="f-sandbox-dir" value="/tmp/haos-green-sandbox">
  </div>
  <div class="field">
    <label>Memory Database Path</label>
    <input type="text" id="f-db-path" value="haos-green.db">
  </div>
</div>`;
...
if [[ "${1:-}" == "--cli" ]]; then
  ...
  read -rp "Sandbox directory [/tmp/haos-green-sandbox]: " SANDBOX
  SANDBOX="${SANDBOX:-/tmp/haos-green-sandbox}"
  read -rp "Memory DB path [haos-green.db]: " DBPATH
  DBPATH="${DBPATH:-haos-green.db}"
