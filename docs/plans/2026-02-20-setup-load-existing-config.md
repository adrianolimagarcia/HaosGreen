...
to:
```rust
println!("   Run the bot with:  cargo run --bin haos-green\n");
```
...
```js
    <input type="text" id="f-sandbox-dir" value="${esc(state.sandbox_dir || '/tmp/haos-green-sandbox')}">
    ...
    <input type="text" id="f-db-path" value="${esc(state.db_path || 'haos-green.db')}">
```
