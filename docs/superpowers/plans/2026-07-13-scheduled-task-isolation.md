...
```rust
            "rerun_scheduled_task" => {
                ...
                // The run ID will be generated by the background runner when the job executes.
...
In `friendly_tool_name()`, after the `"cancel_scheduled_task"` entry, add:
...
At the end of the test module in `tool_notifier.rs`, before the closing `}` of `mod tests`, add:
