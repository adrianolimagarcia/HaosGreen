# Persistent Home Directory (`~/.haos-green`) — Design
...
HaosGreen today scatters its working state and resolves most paths relative to the
process launch directory:

- The sandbox (`[sandbox].allowed_directory`) is a **required** path that users
  typically point at an ephemeral location like `/tmp/haos-green-sandbox`. Files
  the LLM writes there (scripts, programs, notes) are wiped on reboot, so they
  cannot be reused over the long term.
- `haos-green.db`, `skills/`, `agents/`, `supervisor/artifacts`, and the learning
...
- There is no first-class notion of an isolated *instance home*, so running two
  HaosGreen instances on one machine for different purposes requires careful
  manual path juggling.

## Goals

1. Give each HaosGreen instance a single, persistent **home directory**
   (default `~/.haos-green`) that survives reboots.
...
3. Default: `~/.haos-green` (expanded from the OS home directory).
...
~/.haos-green/
  ...
  haos-green.db           # SQLite memory/embeddings                   — OUTSIDE sandbox
...
This instance gets its own `workspace/`, `skills/`, `agents/`, `haos-green.db`,
...
- **Input:** `HAOS_GREEN_HOME` env, `Config` (raw, with `Option<PathBuf>` fields),
  OS home dir.
...
- Because `config.toml`, `haos-green.db`, and OAuth token state live **above**
...
**freeform** — HaosGreen does not impose or enforce subdirectories; the LLM
...
`agents/`) directory does not exist or is empty, HaosGreen seed-copies the
...
- **No automatic file moves.** HaosGreen never relocates an existing
  `./haos-green.db`, `./skills`, etc.
...
- **Actionable startup warning.** When any path is unset *and* a legacy file is
  detected in the launch CWD (e.g. `./haos-green.db`, `./skills/`), or when a path
  ...
  - the exact shell command to copy data
    (e.g. `cp ./haos-green.db ~/.haos-green/haos-green.db`), and
  - the exact `config.toml` line to pin the old path instead.
- **Start-fresh path.** Doing nothing and letting HaosGreen use the new
  `~/.haos-green` defaults is itself the "start fresh" option (a brand-new empty
...
- **Tutorial doc.** Add `docs/persistent-home-directory.md` covering:
  ...
  2. How to **migrate existing data** into `~/.haos-green` (DB, skills, agents,
  ...
  4. How to run **multiple instances** via `HAOS_GREEN_HOME`.
...
- Update `config.example.toml`: comment out the now-optional paths, document the
  `~/.haos-green` defaults, `HAOS_GREEN_HOME`, and `[general].home`.
...
```
HAOS_GREEN_HOME / [general].home / ~/.haos-green
        │
        ▼
...
- `HAOS_GREEN_HOME` set but not absolute → log a warning and ignore it (fall back
  to config/`~/.haos-green`), consistent with XDG's "ignore relative" rule.
...
- Relative `HAOS_GREEN_HOME` is ignored.
...
- Two distinct `HAOS_GREEN_HOME` values yield fully isolated resolved path sets.
...
- **Modify:** `config.example.toml` — document optional paths, `HAOS_GREEN_HOME`,
