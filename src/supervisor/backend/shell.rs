    ...
    async fn run(&self, job: &mut Job, _ctx: &RunContext) -> Result<JobOutput> {
        ...
        let mut child = match &self.isolation {
            ...
            _ => {
                ...
                // **There was a Layer-2 grant refusal here, and it is deleted,
                // not moved.** It compared `job.declared_grants` against the
                // held set and refused a job that declared more than the
                // operator had released. Nothing ever wrote that field — not
                // `submit`, not the planner, not either operator surface — so
                // the set it checked was always empty, the refusal could not
                // fire, and the whole path was inert machinery that read as
                // enforcement. The `declared_grants` field is gone with it, and
                // with it `Grants::missing`, `Grants::covers` and
                // `supervisor::park_reason`, whose only callers were this
                // refusal and its Layer-1 twin.
                //
                // Layer 1 (`Supervisor::shell_gate_reason`) keeps the half that
                // does fire: a task that would select the shell backend is
                // parked for approval when the boundary is absent. That is the
                // ask-for-authorization the operator actually has.
